export const EbrahimiJozves = [
  {
    path: "BQACAgQAAx0Cbu8Z0gACA1ZoqrHxnZlD6haqTFri217Imn65MgAC1RkAAvjiWFHzOsprVEh3_TYE",
    caption: "",
  },
];

export const KhatibiJozves = [
  {
    path: "BQACAgQAAx0Cbu8Z0gACA1hoqrKSZuI-WkNz3Aiesv30rdYbRQACTxYAAiXOSFOsjhzSGaRn7DYE",
    caption: "از صفحه ۱ تا ۴۹ + ۸۳ تا ۱۰۱ مطالعه بشه",
  },
  {
    path: "BQACAgQAAx0Cbu8Z0gACA71oq_Znw-9mjmje73F-CSCdbI4sdwACoB4AApw9MVKDBe_tcZ199jYE",
    caption: "",
  },
];

export const AmeriJozves = [
  {
    path: "BQACAgQAAx0Cbu8Z0gACA2Foqra71f53JGKnLdQ5vlTT4hwn3wACrxUAAjlmwVM9rPsL4uKV1zYE",
    caption: `جزوه دست نویس ترم پیش
سه جلسه اول حذف و از جلسه چهارم مطالعه شود`,
  },
  {
    path: "BQACAgQAAx0Cbu8Z0gACA8Bort3wp0NUMIgfQPvvdTQ-XBSegQACMhsAAoG8eVGVtWSo7aiwFDYE",
    caption: `جزوه خانم طحانیان`,
  },
];

export const NadaliFiles = [
  {
    path: "BQACAgQAAx0Cbu8Z0gACA2NoqrjwR2DfHwRTcviu8vPIU3BptgACRBYAAiXOSFPzjhGQP1cCXTYE",
    caption: "",
  },
  {
    path: "BQACAgQAAx0Cbu8Z0gACA2Voqrj3Gmb6rnNR23k5QydfJZHV4gACRRYAAiXOSFMstWcKeLYbRDYE",
    caption: "",
  },
  {
    path: "BQACAgQAAx0Cbu8Z0gACA2doqrj8skr3JWkgazKGgsj-QlARkAACRhYAAiXOSFMTtKe2VubMhTYE",
    caption: "",
  },
  {
    path: "BQACAgQAAx0Cbu8Z0gACA2loqrkDXVeiSwoksPM4MXtj8N15PwACRxYAAiXOSFMPbM0zdDpimjYE",
    caption: "",
  },
];

export const BagheriFiles = [
  {
    path: "BQACAgQAAx0Cbu8Z0gACA2toqrlkRkrQIiQrMorTmCzUlcUWPAACThYAAiXOSFOtQSlmeMBp-zYE",
    caption: "",
  },
];

export const AshrafiJozves = [
  {
    path: "BQACAgQAAx0Cbu8Z0gACA21oqrmWLEJjxjFGGMRnqEp35IPuWQACzRsAAn1u2FNohLby9AlQjjYE",
    caption: `جزوه دست نویس ترم پیش
 مطالب این جزوه کمی متفاوت با مطالبی هست که در کلاس تدریس شده و بخش گوش رو نداره`,
  },
  {
    path: "BQACAgQAAx0Cbu8Z0gACA79ort28SvpxciBUHS8nK-w9-N22JgACMRsAAoG8eVFDLU1r7Gg-IjYE",
    caption: `جزوه خانم ملک`,
  },
];

export const sample = [
  {
    path: "",
    caption: "",
  },
];
